...
# Introduction
This project is an exploratory data analysis of campaign contributions for the 2012 presidential election in the state of Ohio.  I chose to look at Ohio because it has a history of being a critical swing state for presidential elections and, according to wikipedia, has the longest streak of matching the overall election outcome (since 1960).  It would be a mistake to assume that campaign contributions are a predictor or even loosely correlated with votes (excluding superPACs of course), but Ohio is one of the most frequently visited states on a presidential nominee's campaign trail and for this reason it piqued my interest.   I'll explore the nature of campaign contributions and see if there are any interesting relationships in the data.  I'll also take a look at the geographic distribution of campaign contributions by zipcode to see if there are any geographic relationships.  Anything of significance could be used as a hypothesis in future inferential or predictive analysis.  If there were any useful information that could be extracted from this exploration, it might shed some light on why particular party candidates visit certain areas of Ohio.
...
str(df_test_body)
rm(df_test_body); rm(df_test_header)
```
...
Now we can read in the entire contributions file  We'll have to skip the header again but since we already have the column names from the test dataframe we can set them easily. We add a dummy name to the column names variable then delete that row after we read in the file.
...
Now let's clean the zipcodes to all have 5 numbers and make sure that they are in Ohio.
...
Looks like they are all within the correct range with the exception of the NAs.  This data won't be usable for the choropleth maps but it's still useful for other explorations so we'll keep it.
...
Let's engineer some features to make the data more rich.  Gender from name and party status from candidate.  Also, 

```{r gender-party, echo=FALSE}
#
# ADDING GENDER
...
# get rid of first name and gender names df
df$first_name <- NULL; rm(gender_names_df)
...
```{r add-demo-city, echo=FALSE}

#
# ADDING DEMOGRAPHIC DATA
#
demographics.ohio <- df_zip_demographics %>% filter(region %in% zip.legit)
...
```{r trim-mapdata, echo=FALSE, cache=TRUE, cache.path = './cache/'}
...
None of the numeric variables seem to be strongly correlated with amount. Despite the increase in contributions leading up to the election, the amount is negatively correlated with increasing time.  People may donate larger amounts earlier on in a campaign.
