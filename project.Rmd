...
p1 <- plot_bar_color("as.factor(year(date))")
p2 <- plot_bar_color("month(date, label=TRUE)")
p3 <- plot_bar_color("as.factor(day(date))")
grid.arrange(p1, p2, p3, ncol = 1)
```
...
```{r}
p1 <- ggplot(data = df, aes(x = party, fill = party)) +
  geom_bar() +
  scale_fill_manual(values = c("#377eb8", "#4daf4a", "#e41a1c"), guide = FALSE)
p2 <- ggplot(data = df, aes(x = reorder(candidate, candidate, function(x){length(x)}))) +
  geom_bar() + coord_flip() +
  theme(axis.text.x = element_text(angle=90), axis.title.y = element_blank())
p3 <- ggplot(data = df, aes(x = reorder(candidate, candidate, function(x){length(x)}))) +
  geom_bar() + coord_flip() +
  scale_y_log10() +
  theme(axis.text.x = element_text(angle=90), axis.title.y = element_blank())
...
suppressMessages( grid.arrange(p0, p1, p2, p3, p4, layout_matrix = rbind(c(1,1), c(2,3), c(4,5))) )
```
...
# it has to be created within that geom.
quants <- quantile(total_cont_df$count, probs = c(0, .15, .3, .45, .55, .70, .85, 1))
...
ggplot(data = mapdata.ohio.cont, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(fill = cut(count, breaks = quants, include.lowest = TRUE)), alpha = 0.8) +
  geom_path(colour = 'lightgray') +
  scale_fill_brewer('Total Contributions', palette  = 'Purples') +
...
```{r eval=FALSE}
# size city bubble based upon it's population, visually overwhelming.
if(FALSE){
ggplot(data = mapdata.ohio.cont, aes(x = long, y = lat, group = group)) +
...
# map of total population per zip code
if(FALSE) {
...
ggplot(data = mapdata.ohio.pop, aes(x = long, y = lat, group = group)) +
  ...
  geom_path(colour = 'lightgray') +
  scale_fill_brewer('Total Population', palette  = 'Purples')
}
```
...
```{r}
p1 <- ggplot(data = df, aes(x = month(date, label=TRUE), fill = as.factor(year(df$date)))) +
...
p2 <- ggplot(aes(y = amount, x = as.factor(year(date)), fill = as.factor(year(df$date))), data = df) +
...
```{r fig.height=11, fig.width=8}
total_amt_rep_df <- df[(df$party == "republican"), ] %>% group_by(zipcode) %>% summarise(total_amt = sum(amount))
quants <- quantile(total_amt_rep_df$total_amt, probs = c(0, .15, .3, .45, .55, .70, .85, 1))
mapdata.ohio.totals <- merge(mapdata.ohio, total_amt_rep_df,
                                by.x = 'region', by.y = 'zipcode',
                                all.x = TRUE) %>% arrange(order)

ggplot(data = mapdata.ohio.totals, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(fill = cut(total_amt, breaks = quants, include.lowest = TRUE)), alpha = 0.8) +
  geom_path(colour = 'lightgray') +
  scale_fill_brewer('Total Amount', palette  = 'Reds', guide = FALSE) +
  coord_map(projection = "mercator") +
  geom_point(data = mapdata.ohio.cities,
             aes(x = city_long, y = city_lat, group = NULL),
             size = 2, color = 'blue') +
  geom_text(data = mapdata.ohio.cities,
            aes(x = city_long, y = city_lat, group = NULL, label = city_names), size = 4, color = 'blue', alpha = 1,
            hjust = 1, vjust = -2, fontface = 2) +
  theme_bw()
```
...
ggplot(data = mapdata.ohio.totals, aes(x = long, y = lat, group = group)) +
  geom_polygon(aes(fill = cut(total_amt, breaks = quants, include.lowest = TRUE)), alpha = 0.8) +
  geom_path(colour = 'lightgray') +
  scale_fill_brewer('Total Amount', palette  = 'Blues', guide = FALSE) +
  coord_map(projection = "mercator") +
  geom_point(data = mapdata.ohio.cities,
             aes(x = city_long, y = city_lat, group = NULL),
             size = 2, color = 'red') +
  geom_text(data = mapdata.ohio.cities,
            aes(x = city_long, y = city_lat, group = NULL, label = city_names), size = 4, color = 'red', alpha = 1,
            hjust = 1, vjust = -2, fontface = 2) +
  theme_bw()
```
...
```{r}
df_avg_amt <- df
df_avg_amt <- df_avg_amt %>% group_by(zipcode) %>% #add party to group for ggvis
  summarise(count = n(), total_amt = sum(amount)) %>%
  mutate(avg_amt = total_amt / count)
quants <- quantile(df_avg_amt$avg_amt, probs = c(0, .15, .3, .45, .55, .70, .85, 1))
...
ggplot() +
  ...
  geom_point(data = mapdata.ohio.cities,
             ...
             size = 2, color = 'gold') +
  geom_text(data = mapdata.ohio.cities,
            aes(x = city_long, y = city_lat, group = NULL, label = city_names), size = 4, color = 'black', alpha = 1,
            hjust = 1, vjust = -2, fontface = 2)

# this method works as well but less intuitive with longitude and latitude
if(FALSE){
  ggplot(df_avg_amt, aes(map_id = zipcode, fill = cut(avg_amt, breaks = quants, include.lowest = TRUE))) +
...
anova(m1, m2); anova(m2, m3); anova(m3, m4)
