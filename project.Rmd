...
p1 <- ggplot(data = df, aes(x = amount)) +
  ...
  scale_x_continuous(limits = c(0, 3000),
                     breaks = c(250, 500, 1000, 1500, 2000, 2500))
p2 <- ggplot(data = df, aes(x = amount)) +
...
p3 <- ggplot(data = df, aes(x = candidate)) +
  geom_bar() +
  scale_y_log10() +
  theme(axis.text.x = element_text(angle=90))
...
quants <- quantile(total_cont$count, probs = c(0, .15, .3, .45, .55, .70, .85, 1))
...
ggplot(data = mapdata.ohio.cont, aes(x = long, y = lat, group = group)) +
  ...
  geom_text(data = mapdata.ohio.cities,
            aes(x = city_long, y = city_lat, group = NULL),
                label = city_names, size = 4, color = 'red', alpha = 1,
